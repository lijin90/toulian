...
class DetailAction extends CAction {
    ...
    public function run() {
        ...
        $data = Yii::app()
                ...
                ->queryRow();
        if (!$data) {
            $this->getController()->redirect(Yii::app()->createUrl('training/index'));
        }
        $this->getController()->setPageTitle(Yii::app()->name . ' - 投联培训 - ' . $data['Title']);
        $content = Yii::app()
                ->getDb()
                ->createCommand()
                ->select('c.*')
        ...
        $this->getController()->breadcrumbs = array(
            ...
            array('name' => '投联培训', 'url' => Yii::app()->createUrl('training/index')),
        ...
        $image = Yii::app()
                ...
                ->where(array('and', 'i.AID = :AID', 'i.ImageName = :Name'), array(':AID' => $data['ID'], ':Name' => '培训风采'))
                ->queryAll();
        $this->getController()->render('detail', array(
            'data' => $data,
            'content' => $content,
            'image' => $image
        ));
